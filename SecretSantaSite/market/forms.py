...
class CreateSC(FlaskForm):
    time = DateField('Release Date', format='%Y-%m-%d')
    submit = SubmitField(label='Create')

class JoinSC(FlaskForm):
    invite_code = StringField(label="Invite Code:" , validators=[DataRequired()])
    wishlist = StringField(label="Wishlist:", validators=[DataRequired()])
    submit = SubmitField(label="Join")

class Start(FlaskForm):
    submit = SubmitField(label="Announce")